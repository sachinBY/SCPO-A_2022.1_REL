...
var movingEventDataEntity = vars.entityMap.movingevent[0].movingeventdata[0]
...
(payload.eventGroup  map (eventGroup, index) -> {
	movingevents: (eventGroup.event filter ($.isMovingEvent == true) map (eventData, indexOfEvent) -> {
		...
		avplistUDCS:(flatten([(lib.getUdcNameAndValue(movingEventDataEntity, eventGroup.avpList, lib.getAvpListMap(eventGroup.avpList))[0]) if (eventGroup.avpList != null 
		and (eventGroup.documentActionCode == "ADD" or eventGroup.documentActionCode == "CHANGE_BY_REFRESH")
		and movingEventDataEntity != null
		),
		(lib.getUdcNameAndValue(movingEventDataEntity, eventData.avpList, lib.getAvpListMap(eventData.avpList))[0]) if (eventData.avpList != null 
			and (eventGroup.documentActionCode == "ADD" or eventGroup.documentActionCode == "CHANGE_BY_REFRESH")
			and movingEventDataEntity != null
		)
		])),
